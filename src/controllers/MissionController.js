...
export async function getMission(req, res, next) {
  try {
    ...
    const mission = await MissionService.getMissionDetails(
      userId,
      parseInt(id),
    );
...
export async function updateMission(req, res, next) {
  try {
    ...
    const mission = await MissionService.updateMissionDetails(
      userId,
      parseInt(id),
      validated,
    );
...
export async function deleteMission(req, res, next) {
  try {
    ...
    await MissionService.removeMission(userId, parseInt(id));
    res.json(formatResponse(true, { id: parseInt(id), deleted: true }));
  } catch (error) {
    next(error);
  }
}
