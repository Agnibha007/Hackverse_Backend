...
export async function createMission(userId, missionData) {
  const { title, description, priority, deadline, xp_reward } = missionData;

  const result = await pool.query(
    `INSERT INTO missions (user_id, title, description, priority, deadline, xp_reward, status)
     VALUES ($1, $2, $3, $4, $5, $6, 'pending')
     RETURNING *`,
    [userId, title, description || null, priority, deadline || null, xp_reward],
  );
